 ...
 *     is re-derived from the **published** errors rather than copied from the
 ...
 *
 * `last_status` is **re-derived**, not copied, and the derivation is from the
 * errors that were *published*. Two consequences, both deliberate:
 *
 ...
 */
export function publicHeartbeat(heartbeat, publishedErrors) {
  return {
    last_run_at: heartbeat?.last_run_at ?? null,
    last_status: (publishedErrors ?? []).length > 0 ? 'degraded' : 'ok',
  };
}
...
export function buildPublicSurface({ payload, decisions = [], stack, heartbeat = null, commands = [] }) {
  ...
  if (!validation.ok) {
    ...
    const publicHb = publicHeartbeat(heartbeat, denied.errors);
    return {
      ok: false,
