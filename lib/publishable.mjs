 ...
 * defect behind P2-1 (`ARCHITECTURE-REVIEW.md:216`, ranked #1 open at `:346`).
 *
...
/**
 * Is an error row's `name` published?
 *
 * `gather()` records a failed source as `{ source, name, error }`, where `name`
 * is a package name, an upstream slug or a feed URL depending on which source
 * failed — and carries no ecosystem. So the row is kept when its name is
 * published under *any* of the three lists, and dropped otherwise.
 *
 * The package comparison is deliberately the strict half: with no ecosystem to
 * canonicalise against, a bare entry must match the name exactly. The unsafe
 * direction here is over-matching, because a kept error row publishes the name
 * it failed on; under-matching hides a gap, which the digest's own gaps section
 * already treats as the lesser evil. An entry that declares an ecosystem gets
 * the canonical comparison, because it has said which rule applies.
 */
function nameIsPublished(name, policy) {
  if (typeof name !== 'string') return false;
  const fromPackages = (policy.public_packages ?? []).some((entry) => {
    const parts = entryParts(entry);
