...
/**
 * Advisory lookup — one call per package, issued concurrently.
 *
 * `POST /v1/querybatch` is the tempting endpoint: one call for the whole stack,
 * which is what this function used to do. It is also useless here. The batch
 * response carries `{id, modified}` and nothing else, so every field the mapping
 * in `gather()` reads — `summary`, `severity`, `details`, `affected` — came back
 * null or empty. Measured, not theorised: the response for `express@4.18.2` is
 *
 *     {"results":[{"vulns":[{"id":"GHSA-qw6h-vgh9-j6wx","modified":"…"}]}]}
 *
 * An empty `affected` is not a neutral fact. It reached `rulesDecide()` as "no
 * range to be inside of", which read as "outside the affected range", which is
 * `clear` — so the digest reported **11 clear, 0 to act on** for a stack whose
 * eleven advisories all affect the pinned version and all have a fix published.
 * See the note on `rulesDecide()` for the second half of that failure.
 *
 * `POST /v1/query` takes the same query object and returns the **full** record,
 * `affected[].ranges[]` included. So this is one call per package rather than one
 * per run. OSV publishes no rate limit on either endpoint, and a stack is a
 * handful of packages; the extra calls are cheap and the correctness is not.
 *
 * A package whose query fails is collected rather than thrown: a partial advisory
 * list with a visible gap beats no advisories at all, and the caller renders the
 * gap.
 */
export async function osv(packages) {
  ...
  if (!queries.length) return { results: [], failures: [] };

  const settled = await Promise.allSettled(queries.map((q) => queryOne(q)));
  ...
  settled.forEach((s, i) => {
    if (s.status === 'fulfilled') {
      ...
      results[i] = { vulns: [] };
      failures.push({
        name: packages[i]?.name ?? null,
        ecosystem: packages[i]?.ecosystem ?? null,
...
async function queryOne(query) {
  ...
  throw new SourceError('osv', lastErr?.message ?? 'query failed');
}
...
export async function gather(stack, { token } = {}) {
  ...
  try {
    ...
    // A package whose advisory query failed is a gap, not a quiet package: the
    // digest must not read as "no advisories for this one" when the truth is
    // "we could not ask".
    for (const f of res.failures ?? []) {
      errors.push({ source: 'osv', name: f.name, error: f.error });
    }
