 ...
 *   - **Never logs the credential, and never commits the body.** A failure's
 *     message carries the endpoint's response text so that a caller can classify
 *     it, and `classifyFailure()` reduces that to a status code and a coarse
 *     kind. Callers log the reduction rather than the message: a workflow log is
 *     public on a public repository, and the body is the endpoint's text rather
 *     than ours.
 *
