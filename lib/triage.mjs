...
export function compose({ rules, typed = null, tau = TAU_DEFAULT }) {
  ...
  if (confidence < CONFIDENCE_FLOOR) {
    return {
      decision: DECISIONS.UNCERTAIN,
      reason: `typed layer is below the confidence floor (${confidence} < ${CONFIDENCE_FLOOR})`,
      layer: 'typed',
      evidence: { ...rules.evidence, ...vector },
      tau,
      typed: vector,
  ...
  return {
    ...
    reason:
      decision === DECISIONS.ACT
        ? `typed layer scored ${score.toFixed(2)} ≥ τ ${tau} — treat as reachable`
        : `typed layer scored ${score.toFixed(2)} < τ ${tau} — not reachable enough to act`,
    layer: 'typed',
    evidence: { ...rules.evidence, ...vector },
    tau,
    typed: vector,
