...
/** Truncation for an error body, which stays inside this module. See `reduceTypedError`. */
const ERROR_BODY_CHARS = 200;

/**
 * A typed-layer failure, reduced to what may be **recorded**.
 *
 * The row this lands in is `data/triage.jsonl`: committed, and append-only by
 * design (I5), so a response body written there can never be taken back. That
 * makes this stricter than a log — the same argument `classifyFailure()` makes
 * for a narration failure and `classifyProbeFailure()` for a probe, applied to
 * the one place the body used to reach a file that is kept forever.
 *
 * The cost is real and is stated rather than discovered: a validation failure is
 * now diagnosed by its status alone, and a 422 no longer says which field. The
 * message still carries the detail, so a future caller that wants to print it can
 * — what it must not do is commit it.
 */
export function reduceTypedError(err) {
...
export async function typedDecide(
  ...
  for (let attempt = 0; attempt <= retries; attempt++) {
    ...
    if (!res.ok) {
      // The body is the only thing that says *which* field failed validation, so
      // a bare status code turns a 422 into a guess. It stays in the message —
      // and the message does not escape this module: the caller catches it and
      // records `reduceTypedError()` instead, because the row it lands in is
      // committed and append-only. See that function.
...
export function compose({ rules, typed = null, tau = TAU_DEFAULT }) {
  ...
  // rather than stylistic. `buildFacts()` hands this string to the narrator
  // verbatim, and the reason is part of the document the gate reads — so a number
  // written into a reason becomes a number the gate will *accept*, and a
  // model-derived score would then be printed in the digest as though it were a
  // fact about the world.
  //
  // That is why the rule is asserted rather than trusted. `tests/gate.test.mjs`
  // pins the typed paths' reasons digit-free, and pins `tau` and `typed.*`
  // outside the projected document, so the numbers cannot be laundered into the
  // prose by accident. The tripwire used to be the gate failing at runtime; it is
  // now a test, which fails at review time instead of on a daily cron.
  //
  ...
  for (const advisory of payload.advisories ?? []) {
    ...
    if (rules.decision === DECISIONS.UNCERTAIN && typed.enabled) {
      try {
        ...
        typedError = reduceTypedError(err);
      }
    }
