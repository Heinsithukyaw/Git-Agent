...
export function renderDigest({ payload, decisions, narration = null, narrationStatus = null, commands = [] }) {
  ...
  const narrationGap = Boolean(narrationStatus?.configured && !narrationStatus.ok);
  ...
  const narrationSilent = Boolean(narration && !body);
  if (errors.length || narrationGap || narrationSilent) {
    ...
    if (narrationGap) {
      out.push(`- \`narration\` — ${narrationGapLine(narrationStatus)}`);
    } else if (narrationSilent) {
      out.push(`- \`narration\` — ${narrationGapLine({ kind: 'title-only' })}`);
    }
    out.push('');
  }
