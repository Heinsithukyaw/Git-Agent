  ...
  switch (verb) {
    case 'why': {
      ...
      const lines = rows.map((d) => {
        ...
        if (d.typed) {
          // No `confidence` here: a `noul` answer has no such field, so the
          // only honest thing to print is the answer, the score derived from
          // it, and the band the score was read against.
          parts.push(
            `  - typed layer: reachability ${fmt(d.typed.reaches_a_trust_boundary)}, ` +
              `component ${fmt(d.typed.we_use_the_vulnerable_component)}, ` +
              `score ${fmt(d.typed.score)}, τ ${d.tau} (band ${fmt(1 - d.tau)}–${fmt(d.tau)} escalates)` +
