...
function narrationGapLine({ kind, status }) {
  ...
  switch (kind) {
    ...
    // The step itself, not the call. `crashed` is a top-level `catch` — something
    // threw before or around the call — and `started` is the provisional record,
    // which survives only if the process was killed without reaching a handler.
    case 'crashed':
      return `the narration step failed before it could call the model — narration was skipped.${suffix}`;
    case 'started':
      return `the narration step did not finish — it was interrupted before it recorded an outcome.${suffix}`;
...
export function narrationGap({ narration = null, narrationStatus = null } = {}) {
  // `configured: false` is the one un-successful record that is not a gap: no
  // endpoint was asked for, and a keyless instance is a complete instance (I9).
  // Everything else that did not succeed is a gap — including `configured: null`,
  // which is the provisional record `narrate-step` writes before it can read its
  // own configuration, and which means the step began and never got far enough to
  // say what happened. Stated as "not `false`" rather than "is `true`" so a record
  // that is missing the field cannot pass by accident.
  if (narrationStatus && narrationStatus.ok === false && narrationStatus.configured !== false) {
...
export function renderDigest({ payload, decisions, narration = null, narrationStatus = null, commands = [] }) {
  ...
  if (commands.length) {
    ...
    for (const c of commands) {
      // The verb and the argument, never the author. This document is published
      // twice over — committed, and rendered on Pages — and a login belongs to a
      // person rather than to this repository. The actor is still recorded, in
      // `history/commands.jsonl`: that is the audit trail, and the one place the
      // question "who asked for this?" has to stay answerable.
      out.push(`- \`/agent ${c.verb}${c.arg ? ` ${c.arg}` : ''}\` — ${c.outcome ?? 'recorded'}`);
    }
    out.push('');
  }
