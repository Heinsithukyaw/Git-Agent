...
/**
 * I16 — the public renderer reads no private path.
 *
 ...
 * projected.
 *
 * That makes its **read set** the security boundary. `data/summary.json` is the
 * private summary and `history/commands.jsonl` rows carry a GitHub login
 * (`lib/commands.mjs:24`); everything this script reads is written to a
 * world-readable Pages artifact. A regression here is silent — the page renders,
 * the run is green, and the only symptom is a name that should not be public.
 *
 * Four assertions, all textual and deliberately narrow:
 *
 *   1. the private summary is not named;
 *   2. the command log is not named;
 *   3. `PUBLIC_SUMMARY` is *declared* as a public path. The literal scan above
 *      cannot see a read through a constant, and the constant is the shipped
 *      read — so its declaration is read out of `lib/public-surface.mjs`, the way
 *      I4 reads `GATE_EXEMPT` out of `lib/store.mjs` and I13 reads the author
 *      allowlist out of the script. Importing the value here would make this
 *      assertion impossible to see fail, which is the same as not having it;
 ...
 *
 * **An import is not a use.** `import { PUBLIC_DIGEST_PREFIX }` followed by
 * `readdirSync('digest')` narrows nothing, and the marker sitting in the import
 * list read as a use — so assertion 4 was satisfiable by a file that imported the
 * prefix and never applied it. Import lines are excluded from that search, which
 * the synthetic must-fail case pins.
 *
 * **Non-vacuous by construction.** A file that names no `public-` digest fails
 * assertion 4's second half, and a surface that declares no summary path fails
 * assertion 3, so an empty or unreadable renderer cannot pass this check by
 * having nothing to find.
 *
 * **What it cannot see.** A private path assembled from a variable, and any file
 * the renderer reads that is not one of the three named here. It guards the
 * regression that is easy to make; it is not a proof that the read set is closed.
 */
const PUBLIC_RENDERER = 'scripts/render-site.mjs';
const PUBLIC_SURFACE = 'lib/public-surface.mjs';
const FORBIDDEN_PUBLIC_PATHS = new Set(['data/summary.json', 'history/commands.jsonl']);
const PRIVATE_RENDERER_PATHS = [
  { re: /['"`]data\/summary\.json['"`]/, what: 'data/summary.json (the unprojected summary)' },
  { re: /['"`]history\/commands\.jsonl['"`]/, what: 'history/commands.jsonl (its rows carry the author login)' },
];
...
export function checkPublicRendererReadsNoPrivatePath(root = process.cwd()) {
  ...
  // Executable text only. The module header names the private paths it forbids,
  // and a rule that fired on the paragraph explaining it could not be documented
  // — the same reason I8 scans code and not comments.
  const text = stripCommentLines(fs.readFileSync(abs, 'utf8'));
  const violations = [];

  for (const { re, what } of PRIVATE_RENDERER_PATHS) {
    if (re.test(text)) {
      violations.push({ file: PUBLIC_RENDERER, rule: `I16: the public renderer reads ${what}` });
    }
  }

  // The literal scan cannot see a read through a constant, so the constant's
  // declaration is read from the source. See assertion 3 above: importing the
  // value instead would make this branch unreachable from a test.
  ...
  if (!declared) {
    violations.push({
      file: PUBLIC_SURFACE,
      rule: 'I16: PUBLIC_SUMMARY is not declared, so the public summary path is unasserted',
    });
  } else if (FORBIDDEN_PUBLIC_PATHS.has(declared[1])) {
    violations.push({
      file: PUBLIC_SURFACE,
      rule: `I16: PUBLIC_SUMMARY names a private path (${declared[1]})`,
    });
  }

  // An import is not a use. See the header: the marker in an import list used to
  // satisfy this, so `readdirSync('digest')` next to an unused import passed.
  const used = text
    .split('\n')
    .filter((l) => !/^\s*import\b/.test(l))
    .join('\n');
  const namesPrefix =
    used.includes('PUBLIC_DIGEST_PREFIX') ||
    used.includes(`'${PUBLIC_DIGEST_PREFIX}'`) ||
    used.includes(`"${PUBLIC_DIGEST_PREFIX}"`);

  if (/readdirSync\([^)]*['"`]digest['"`]/.test(text) && !namesPrefix) {
    violations.push({
  ...
  if (!namesPrefix) {
    violations.push({
      file: PUBLIC_RENDERER,
      rule: `I16: the public renderer names no ${PUBLIC_DIGEST_PREFIX} digest, so it cannot be reading the projected surface`,
    });
  }
