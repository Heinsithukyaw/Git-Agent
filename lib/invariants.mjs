 ...
 * no single literal to resolve. A `..` segment is excluded from the candidate
 * test, so `data/../data/stack.json` is not reported either. These are real gaps,
 * and they are **asserted as gaps** in `tests/invariants.test.mjs` rather than
 * left for someone to find. The completeness half is not static at all: the
 * pipeline canary runs the real commit step and the real renderer over a sentinel
 * private package and sweeps every published byte, which is what catches a filter
 * that runs and is given the wrong content. This check guards the regressions
 * that are easy to make; it is not a proof that the read set is closed.
 */
 ...
 * otherwise permit. The permitted set is derived from the declaration, so without
...
function isCandidatePath(literal) {
  return PATH_CHARSET_RE.test(literal) && !literal.includes('..') && resolvesIntoCollectorRoot(literal);
}
...
export function checkPublicRendererReadsNoPrivatePath(root = process.cwd()) {
  ...
  for (const { fn, target } of readCallTargets(text)) {
    ...
    const join = target.match(/^path\.join\(\s*([^,]+?)\s*(?:,|\))/);
    if (join) {
      const folded = literalValue(join[1].trim(), consts);
      if (folded !== null && isProjected(folded)) continue;
      // The root resolves into a collector root, so assertion 1 reports it and
      // naming it again here would be the same finding twice.
      if (folded !== null && isCandidatePath(folded)) continue;
      violations.push({
        file: PUBLIC_RENDERER,
        ...
        rule:
          folded === null
            ...
            : `I16: ${fn} is given ${target}, whose root ${folded} is not a projected artifact`,
      });
      continue;
    }
