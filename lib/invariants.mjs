 ...
 * Four assertions:
 *
 *   1. **Every read target is a projected artifact.** The renderer may read the
 *      declared `PUBLIC_SUMMARY` and `digest/`, and nothing else. Three spellings
 *      of a read target are resolved, because a scan that saw only one of them
 *      would be a denylist wearing an allowlist's clothes: a literal passed to a
 *      read call, a literal bound to a `const` — which is how a target hides from
 *      the first — and a two-literal `path.join`. URLs are excluded: they are
 *      slashes without being paths.
 ...
 *      allowlist in assertion 1 is *derived* from this declaration, so it needs
 *      its own guard: re-pointing the constant would otherwise widen the
 *      allowlist to match.
 *   3. **The digest directory, if enumerated, is narrowed to the shared
 ...
 *      than a decision.
 *   4. **The narrowing is applied, not merely mentioned.** The prefix must appear
 ...
 * in executable code fails assertion 1's floor, and a surface that declares no
 * summary path fails assertion 2 — so an empty or unreadable renderer cannot pass
 * this check by having nothing to find.
 *
 * **What it cannot see, named rather than discovered.** A target built by a
 * `path.join` with a non-literal component (`path.join(dir, name)` where `dir` is
 * a variable), or computed at run time, is invisible to all three spellings. This
 * guards the regressions that are easy to make, including the one a verifier
 * made, and it is not a proof that the read set is closed.
 */
...
/**
 * Paths that must never be the declared summary, whatever the allowlist would
 * otherwise permit. The allowlist is derived from the declaration, so without
 * this a re-pointed constant would simply widen it to match.
 */
const FORBIDDEN_PUBLIC_PATHS = new Set(['data/summary.json', 'data/stack.json', 'history/commands.jsonl']);

/** A literal passed to something that reads. */
const READ_CALL_RE =
  /\b(?:readFileSync|readFile|readdirSync|existsSync|readJson|readJsonIfExists|readRows|statSync)\s*\(\s*['"`]([^'"`\n]+)['"`]/g;
/** A literal bound to a name — the first way a read target hides from the scan above. */
const PATH_CONST_RE = /\bconst\s+[A-Za-z_$][\w$]*\s*=\s*['"`]([^'"`\n]*\/[^'"`\n]*)['"`]/g;
/** `path.join('a', 'b')` with both components literal — the second way. */
const PATH_JOIN_LITERAL_RE = /path\.join\(\s*['"`]([^'"`\n]+)['"`]\s*,\s*['"`]([^'"`\n]+)['"`]/g;
/** The statement that enumerates the digest directory, up to its `;`. */
const DIGEST_ENUM_RE = /readdirSync\(\s*['"`]digest['"`]\s*\)[\s\S]*?;/;
/** A URL has slashes without being a repository path. */
...
export function checkPublicRendererReadsNoPrivatePath(root = process.cwd()) {
  ...
  // ---- assertion 2: the declaration the allowlist is derived from ----------
  ...
  // ---- assertion 1: every read target is a projected artifact -------------
  const allowed = (p) => p === declared?.[1] || p === 'digest' || p.startsWith('digest/');
  const targets = [];
  for (const m of text.matchAll(READ_CALL_RE)) targets.push({ target: m[1], how: 'a read' });
  for (const m of text.matchAll(PATH_CONST_RE)) targets.push({ target: m[1], how: 'a path bound to a name' });
  for (const m of text.matchAll(PATH_JOIN_LITERAL_RE)) {
    targets.push({ target: `${m[1]}/${m[2]}`, how: 'a joined path' });
  }
  ...
  for (const { target, how } of targets) {
    ...
    if (!allowed(norm)) {
      violations.push({
        ...
        target,
        rule: `I16: the public renderer names ${target} as ${how}, which is not a projected artifact`,
      });
    }
  }

  if (!used.includes('PUBLIC_SUMMARY')) {
    violations.push({
      file: PUBLIC_RENDERER,
      rule: 'I16: the public renderer never reads PUBLIC_SUMMARY, so it is not reading the projected surface',
    });
  }

  // ---- assertions 3 and 4: the digest narrowing is applied -----------------
