 ...
 * only then uploads.
 *
 * An empty page set therefore **fails closed**. `site/` is committed and the
 * render step precedes this in both workflows, so no pages means the check could
 * not look — which is not the same as having looked and found nothing. Returning
 * ok here would let the rule stop being enforced, silently, the moment `site/`
 * left the tree or the render step was dropped from a workflow.
 */
export function checkPagesHoldNoKey(root = process.cwd()) {
  ...
  if (!pages.length) {
    return {
      ok: false,
      violations: [
        { file: 'site', rule: 'I10: no page to check — render the site before running this' },
      ],
    };
  }
  ...
  return {
    ...
    detail: `${pages.length} page(s) checked`,
  };
}
