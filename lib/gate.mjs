 ...
 *
 * THE INPUT IS THE NARRATOR'S INPUT. The payload handed to `check()` must be the
 * document the narrator was shown the facts of, not a different one that happens
 * to be related. A gate reading document A while the narrator read document B
 * will reject correct prose for stating something B knew and A did not — and it
 * will do it every run, deterministically, which is how it stayed invisible: the
 * facts carried a per-decision `severity` computed from OSV's CVSS vector, the
 * payload carried only the vector string, and 10 of 16 facts were ungroundable.
 * `attachDecisions()` in `lib/render.mjs` builds that single document, and
 * `buildFacts()` takes one argument so it cannot reach a second one.
 *
...
//
// Two further rules, both learned from a live run rather than from reasoning:
//
//   1. **The capture must end on an alphanumeric.** `[a-z0-9._-]` includes `.`,
//      so a name at the end of a sentence captured its full stop — `we import
//      express.` produced the entity `express.`, which no payload can contain.
//      `SCOPED_RE` and `SEPARATED_RE` already ended on an alphanumeric and say
//      why in a comment; these two patterns had not been given the rule.
//   2. **A copula is not a state verb.** With `is|are|was|…` accepted as the
//      predicate, any noun followed by "is" became a package: `the same
//      underlying issues is not stated` produced the entity `issues`, and `the
//      upgrade is available` produced `upgrade`. A copula now has to be
//      *followed* by a state verb, so "lodash is pinned" still matches and
//      "issues is not stated" does not.
const PACKAGE_KEYWORD_RE =
  /\b(?:packages?|dependenc(?:y|ies)|depend(?:s|ing|ed)?|modules?|librar(?:y|ies)|libs?|crates?|gems?|import(?:s|ed|ing)?|require[sd]?|bump(?:ed|s)?|upgrade[sd]?|downgrade[sd]?|pin(?:ned|s)?|install(?:s|ed|ing)?|use[sd]?|using)\s+`?([a-z][a-z0-9._-]{0,38}[a-z0-9])`?/gi;

const PACKAGE_STATE_RE =
  /(?<![A-Za-z0-9@/_.-])`?([a-z][a-z0-9._-]{0,38}[a-z0-9])`?\s+(?:(?:is|are|was|were|be|being|has|have|had)\s+)?(?:pinned|affected|imported|reachable|vulnerable|fixed|released|upgraded|deprecated|unmaintained|watched|clear|flagged)\b/gi;
...
//
// The list doubles as this project's own reason vocabulary — `reachability`,
// `exposure`, `symbol`, `threshold`, `confidence`, `layer`, `range` — because
// `buildFacts()` hands the decision reasons to the narrator verbatim, and the
// narrator is expected to be able to repeat them. Every entry here is a small
// weakening: a package genuinely named `usage` would go unchecked. The tradeoff
// is worth it for words that appear in our prose as common nouns, and
// `tests/gate.test.mjs` asserts that every fact the narrator receives is
// passable, so the next missing word fails a test instead of a run.
const STOPWORDS = new Set([
  ...
  'under', 'until', 'up', 'us', 'usage', 'use', 'used', 'uses', 'using', 'version', 'versions', 'very', 'via',
 ...
 *
 * The payload is the primary source, and it is expected to be the narrator's
 * input — see the header. `derived` is the escape hatch for scalars the *caller*
 * computed and the narrator was never shown, such as the counts in the digest
 * header. Those are arithmetic over the payload rather than invention, so they
 * are legitimate — but they must be passed explicitly, never assumed, and they
 * should not be used to paper over a fact the narrator was handed. A fact the
 * narrator can see belongs in the document.
 */
